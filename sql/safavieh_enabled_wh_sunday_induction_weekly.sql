...
-- Weeks: Sunday-start (aligned with weekend shipping enablement lookback)
-- Enable date reference: 2026-07-07
...
weekly AS (
  SELECT
    ...
    END) AS sun_induct_vol,
    COUNT(DISTINCT CASE
      WHEN induction_dow_adj IN (6, 7) THEN ops
    END) AS weekend_induct_vol,
    ROUND(
...
l6w_pre_enable AS (
  SELECT
    supplier_id,
    su_name,
    ROUND(AVG(pct_sun_induct), 4) AS l6w_pre_pct_sun,
    ROUND(AVG(pct_weekend_induct), 4) AS l6w_pre_pct_weekend,
...
SELECT
  ...
  w.fri_sat_vol,
  w.sun_induct_vol,
  w.weekend_induct_vol,
  w.pct_sun_induct,
  w.pct_weekend_induct,
  p.l6w_pre_pct_sun,
