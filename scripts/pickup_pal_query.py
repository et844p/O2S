...
def format_typical_days(df) -> str:
    ...
    for _, row in df.iterrows():
        lines.append(
            ...
            f"avg {row['avg_trucks_when_scheduled'] if row['avg_trucks_when_scheduled'] == row['avg_trucks_when_scheduled'] else 0} trucks "
            f"({row['pickup_frequency'].lower()})"
        )
    return "\n".join(lines)
...
def main() -> int:
    ...
    sql = load_sql(args.query, args.supplier)
    df = query_df(sql)

    if args.raw:
        print(df.to_string(index=False))
    else:
        print(FORMATTERS[args.query](df))
