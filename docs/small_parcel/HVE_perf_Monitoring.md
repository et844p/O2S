...
| `det_delivery_date` | DATE | Deterministic delivery date from the on-site promise model |
| `det_o2d` | INT | Deterministic O2D (days). Prefer the table column when present; else `DATE_DIFF(det_delivery_date, order_complete_date, DAY)` |
| `det_del_rel` | INT | Boolean (1/0): deterministic reliability vs the ML / on-site promise |
